...
import { generateDonationLink } from '@/lib/high-school/billing'
...
export async function POST(
  ...
  try {
    ...
    const result = await generateDonationLink(params.id)

    if (!result.success) {
      return NextResponse.json(
        { error: result.error || 'Failed to generate donation link' },
        { status: 400 }
      )
    }
    ...
    return handleApiError(error, 'Failed to generate donation link')
  }
}
