...
export default function Step1SignUp({ onComplete }: Step1SignUpProps) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.removeItem('scout_onboarding') // Ensure no stale scout flag
      }

      if (mode === 'signup') {
        const currentOrigin = typeof window !== 'undefined' ? window.location.origin : ''
        if (!currentOrigin) throw new Error('Cannot determine current origin')
