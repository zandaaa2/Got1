...
interface RoleSelectionModalProps {
  isOpen: boolean
  onClose: () => void
  onSelect: (role: 'player' | 'scout' | 'skip') => void
}

export default function RoleSelectionModal({ isOpen, onClose, onSelect }: RoleSelectionModalProps) {
  const [selectedRole, setSelectedRole] = useState<'player' | 'scout' | 'skip' | null>(null)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="What would you like to do on Got1?">
      <div className="space-y-4">
        ...
        >
          <div className="flex items-start gap-3">
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5 ${
              selectedRole === 'skip' ? 'border-black bg-black' : 'border-gray-300'
            }`}>
              {selectedRole === 'skip' && (
                <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-black mb-1">Skip for Now</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                Just browsing? You can always add player or scout information later from your profile settings.
              </p>
            </div>
          </div>
        </button>
