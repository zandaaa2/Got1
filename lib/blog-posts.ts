...
// Fallback blog posts (used if database is not available)
export const fallbackBlogPosts: BlogPost[] = [
  {
    slug: 'recruiting-trends-2026',
    title: 'Top Recruiting Trends for 2026',
    excerpt: 'Discover the latest trends shaping college football recruiting and how to position yourself for success in the evolving landscape.',
    content: `# Top Recruiting Trends for 2026
...
Start building your recruiting profile today and get the professional feedback you need to advance your football career.`,
    image: '/landingpage/herohorizontal.jpg',
    author: 'Zander Huff',
    authorEmail: 'zander@got1.app',
    publishedAt: '2024-12-15T10:00:00Z',
  },
  {
    slug: 'how-scouts-evaluate-qbs',
    title: 'How Scouts Evaluate QBs',
    excerpt: 'Learn what college scouts are looking for when evaluating quarterback prospects and how to showcase your skills effectively.',
    content: `# How Scouts Evaluate QBs
...
Ready to get professional QB evaluation? Submit your film to verified college scouts on Got1 and receive detailed feedback on your game.`,
  ...
  },
  {
    ...
    title: 'Success Stories from Got1 Athletes',
    excerpt: 'Read inspiring stories from athletes who used Got1 evaluations to advance their recruiting and achieve their college football dreams.',
    content: `# Success Stories from Got1 Athletes
...
Ready to advance your recruiting journey? Submit your film today and get professional evaluations from verified college scouts.`,
  ...
  },
  {
    ...
    excerpt: 'A comprehensive guide to the college football recruiting timeline and when to take key actions throughout your high school career.',
    content: `# Understanding the Recruiting Timeline
...
Ready to get started? Submit your film for professional evaluation today.`,
  ...
  {
    slug: 'game-film-best-practices',
    title: 'Game Film Best Practices',
    excerpt: 'Expert tips on creating compelling game film that captures scouts\' attention and effectively showcases your abilities on the field.',
    content: `# Game Film Best Practices
...
export async function getAllBlogPosts(): Promise<BlogPost[]> {
  try {
    ...
    if (error || !data || data.length === 0) {
      // Fallback to static data
      return fallbackBlogPosts.sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
    }

    // Sort by pinned first, then by published_at
    const sortedData = [...data].sort((a, b) => {
      // First sort by pinned (pinned posts first)
      if (a.pinned && !b.pinned) return -1
      if (!a.pinned && b.pinned) return 1
      // Then sort by published_at (most recent first)
      const dateA = new Date(a.published_at || a.created_at || 0).getTime()
      const dateB = new Date(b.published_at || b.created_at || 0).getTime()
      return dateB - dateA
    })

    return sortedData.map(post => ({
      id: post.id,
      slug: post.slug,
      title: post.title,
